...
React-приложение находится в `apps/web`.

```sh
npm ci
npm run dev                  # API: localhost:4000
npm run dev -w @checkout/web # интерфейс: localhost:5173
npm run build -w @checkout/web
```

Клиент API собран в одном модуле: он добавляет базовый адрес, авторизацию и JSON-заголовки, единообразно разбирает транспортные, HTTP и JSON-ошибки. Компоненты получают уже типизированные данные или готовое сообщение ошибки.

Сессия, текущий заказ и токен сохраняются в `localStorage`. Корзина всегда перечитывается после изменения на сервере. Расчёт создаётся при смене корзины или доставки. Создание заказа и попытки оплаты используют ключи идемпотентности. Статус оплаты опрашивается только пока он не завершён и таймер очищается при уходе со страницы.

Проверены: добавление/удаление товаров, самовывоз и курьер, карта с успешным и отклонённым сценарием, наличные, повторная оплата, пустая корзина и обновление страницы на оформленном заказе.

В отображении корзины используется один линейный поиск только при нажатии «Добавить», а серверная корзина принимается как источник истины. Пересчёт totals вручную не выполняется: интерфейс использует значения API, без дополнительных обходов массива позиций.
